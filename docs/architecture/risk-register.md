...
| P1 | High-refresh scrolling-waveform presentation needs physical GPU verification | Math/image tests cover full physical-column density, stable tile slots, filter gutters, DPR centring and disposable worker generations. They cannot prove compositor timing or driver-specific sampling at 59.94–144 Hz and mixed DPR. | Run the scrolling-waveform matrix in `docs/testing/regression-checklist.md` on each supported graphics API; record refresh rate, DPR and render stats. | open |
